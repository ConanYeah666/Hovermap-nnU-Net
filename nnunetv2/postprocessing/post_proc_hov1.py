...
####
def __proc_np_hv(focus, hov, hov2, hov3):
    ...
    #tif.imwrite("/data/zucksliu/Glom-Segmnentation/media/fabian/post/G3overall1.tif", overall)
    """mu = np.mean(overall)
    ...
    overall = np.array(overall >= 0.5, dtype=np.int16)

    marker = blb - overall
    tif.imwrite("D:/marker1.tif", marker)
    marker[marker < 0] = 0
    #tif.imwrite("/data/zucksliu/Glom-Segmnentation/media/fabian/post/G35marker2.tif", marker)
    ...
    tif.imwrite("D:/marker3.tif", marker)
    print("Start marker 3")
    marker = measurements.label(marker)[0]
    print("Start marker 4")
    marker = remove_small_objects(marker, min_size=50)
    print("Start Final pred")
    pred_inst = watershed(blb, markers=marker, mask=blb, watershed_line=True)

    return pred_inst

print("Start tif read")
#hov = tif.imread("/data/zucksliu/Glom-Segmnentation/media/fabian/nnuNet_results/Dataset042_Glom3Hov/nnUNetTrainer_20epochs__nnUNetPlans__3d_lowres/fold_all/validation/Glom_001.tif")
hov1 = tif.imread('D:/Glom_001.tif')
hov2 = tif.imread('D:/Glom_001y.tif')
hov3 = tif.imread('D:/Glom_001z.tif')
print("Start tif read2")
#focus = tif.imread("/data/zucksliu/Glom-Segmnentation/media/fabian/nnuNet_results/Dataset041_GlomFullStack6Sample/nnUNetTrainer_20epochs__nnUNetPlans__3d_cascade_fullres/fold_all/validation/Glom_000.tif")
focus = tif.imread('D:/Glom_001_raw.tif')
pred = __proc_np_hv(focus, hov1, hov2, hov3)
print("Start tif write")
#tif.imwrite("/data/zucksliu/Glom-Segmnentation/media/fabian/post/5Glom_001.tif",pred)
