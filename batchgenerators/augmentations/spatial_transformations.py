...
def augment_spatial(data, seg, focus, patch_size, patch_center_dist_from_border=30,
    ...
    if seg is not None:
        if dim == 2:
            ...
            seg_result = np.zeros((seg.shape[0], seg.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                  dtype=np.float32)
            focus_result = np.zeros((focus.shape[0], focus.shape[1], patch_size[0], patch_size[1], patch_size[2]),
                                  dtype=np.float32)
    ...
    for sample_id in range(data.shape[0]):
        ...
        if modified_coords:
            ...
            for channel_id in range(focus.shape[1]):
                print("focus shape", focus.shape, sample_id, channel_id)
                focus_result[sample_id, channel_id] = interpolate_img(focus[sample_id, channel_id], coords, order_seg,
                                                                    border_mode_seg, cval=border_cval_seg,
                                                                    is_seg=True)

                #print("focus1 result:", np.max(focus), np.min(focus) ,"focus2 result:", np.max(focus_result), np.min(focus_result))
            if seg is not None:
                for channel_id in range(seg.shape[1]):
                    ...
                    seg_result[sample_id, channel_id] = interpolate_img(seg[sample_id, channel_id], coords, order_seg,
                                                                        border_mode_seg, cval=border_cval_seg,
                                                                        is_seg=False)
                    #print("seg result:" , np.max(seg_result), np.min(seg_result))
            ...
            if seg is None:
                ...
                f = focus[sample_id:sample_id + 1]
            if random_crop:
                margin = [patch_center_dist_from_border[d] - patch_size[d] // 2 for d in range(dim)]
                d, s, f = random_crop_aug(data[sample_id:sample_id + 1], s, f, patch_size, margin)
            else:
                d, s, f = center_crop_aug(data[sample_id:sample_id + 1], patch_size, s, f)
            data_result[sample_id] = d[0]
            if seg is not None:
                seg_result[sample_id] = s[0]
            if focus is not None:
                focus_result[sample_id] = f[0]
    """import tifffile as tif
    tif.imwrite(
        "/data/zucksliu/Glom-Segmnentation/media/fabian/nnuNet_results/Dataset040_GlomHovTest/nnUNetTrainer_20epochs__nnUNetPlans__3d_fullres/test/seg1.tif",
        seg)
    tif.imwrite(
        "/data/zucksliu/Glom-Segmnentation/media/fabian/nnuNet_results/Dataset040_GlomHovTest/nnUNetTrainer_20epochs__nnUNetPlans__3d_fullres/test/seg2.tif",
        seg_result)
    tif.imwrite(
        "/data/zucksliu/Glom-Segmnentation/media/fabian/nnuNet_results/Dataset040_GlomHovTest/nnUNetTrainer_20epochs__nnUNetPlans__3d_fullres/test/focus1.tif",
        focus)
    tif.imwrite(
        "/data/zucksliu/Glom-Segmnentation/media/fabian/nnuNet_results/Dataset040_GlomHovTest/nnUNetTrainer_20epochs__nnUNetPlans__3d_fullres/test/focus2.tif",
        focus_result)
    tif.imwrite(
        "/data/zucksliu/Glom-Segmnentation/media/fabian/nnuNet_results/Dataset040_GlomHovTest/nnUNetTrainer_20epochs__nnUNetPlans__3d_fullres/test/data1.tif",
        data)
    tif.imwrite(
        "/data/zucksliu/Glom-Segmnentation/media/fabian/nnuNet_results/Dataset040_GlomHovTest/nnUNetTrainer_20epochs__nnUNetPlans__3d_fullres/test/data2.tif",
        data_result)"""
    print("Seg done")
    # exit()
    return data_result, seg_result, focus_result
