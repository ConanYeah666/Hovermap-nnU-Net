...
class nnUNetDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
                 ...
                 folder_with_segs_from_previous_stage2: str = None,
                 folder_with_segs_from_previous_stage3: str = None):
        """
        ...
        for c in case_identifiers:
            ...
            if folder_with_segs_from_previous_stage2 is not None:
                print('previous stage_2 found: .... ')
                self.dataset[c]['seg_from_prev_stage_file2'] = join(folder_with_segs_from_previous_stage2, "%s.npz" % c)
            if folder_with_segs_from_previous_stage3 is not None:
                print('previous stage_3 found: .... ')
                self.dataset[c]['seg_from_prev_stage_file3'] = join(folder_with_segs_from_previous_stage3, "%s.npz" % c)
    ...
    def load_case(self, key):
        ...
        if 'open_data_file' in entry.keys():
            ...
            print("using second way open data")
        ...
        print("Done data:", data.shape)

        if 'open_seg_file' in entry.keys():
            seg = entry['open_seg_file']
            print('using open seg file')
        elif isfile(entry['data_file'][:-4] + "_seg.npy"):
            print("using second way open seg")
            seg = np.load(entry['data_file'][:-4] + "_seg.npy", 'r')
            if self.keep_files_open:
                self.dataset[key]['open_seg_file'] = seg
                # print('saving open seg file')
        else:
            seg = np.load(entry['data_file'], mmap_mode='r')['seg']
        # print(seg)
        print("Done seg:", seg.shape)
        #print("Seg max min:", np.max(seg), np.min(seg))
        if 'open_focus_file' in entry.keys():
            focus = entry['open_focus_file']
            print('using open focus file')
        elif isfile(entry['data_file'][:-4] + "_focus.npy"):
            print("using second way open seg")
            focus = np.load(entry['data_file'][:-4] + "_focus.npy", 'r')
            if self.keep_files_open:
                self.dataset[key]['open_focus_file'] = focus
                # print('saving open seg file')
        else:
            focus= np.load(entry['data_file'], mmap_mode='r')['focus']
        print("Done focus:", focus.shape)
        #print("Focus1 max min:", np.max(focus[0,:,:,:]), np.min(focus[0,:,:,:]))
        #print("Focus2 max min:", np.max(focus[1, :, :, :]), np.min(focus[1, :, :, :]))
        #print("Focus3 max min:", np.max(focus[2, :, :, :]), np.min(focus[2, :, :, :]))
        ...
        if 'seg_from_prev_stage_file' in entry.keys():
            if isfile(entry['seg_from_prev_stage_file'][:-4] + ".npy"):
                ...
                seg_prev = np.load(entry['seg_from_prev_stage_file'], mmap_mode='r')['seg']
            seg1 = np.vstack((seg, seg_prev[None]))
            del seg_prev,seg
            gc.collect()
            seg = seg1

        if 'seg_from_prev_stage_file2' in entry.keys():
            if isfile(entry['seg_from_prev_stage_file2'][:-4] + ".npy"):
                seg_prev = np.load(entry['seg_from_prev_stage_file2'][:-4] + ".npy", 'r')
            else:
                seg_prev = np.load(entry['seg_from_prev_stage_file2'])['seg']
            seg1 = np.vstack((seg, seg_prev[None]))
            del seg_prev, seg
            gc.collect()
            seg = seg1

        if 'seg_from_prev_stage_file3' in entry.keys():
            if isfile(entry['seg_from_prev_stage_file3'][:-4] + ".npy"):
                seg_prev = np.load(entry['seg_from_prev_stage_file3'][:-4] + ".npy", 'r')
            else:
                seg_prev = np.load(entry['seg_from_prev_stage_file3'])['seg']
            seg1 = np.vstack((seg, seg_prev[None]))
            del seg_prev, seg
            gc.collect()
            seg = seg1
        ...
        return data, seg, focus, entry['properties']
