...
class PreprocessAdapter(DataLoader):
    def __init__(self, list_of_lists: List[List[str]], list_of_segs_from_prev_stage_files: Union[List[None], List[str]],
                 list_of_segs_from_prev_stage_files2: Union[List[None], List[str]],
        ...
        super().__init__(list(zip(list_of_lists, list_of_segs_from_prev_stage_files + list_of_segs_from_prev_stage_files2 , output_filenames_truncated)),
    ...
    def generate_train_batch(self):
        ...
        data, seg, focus, data_properites = self.preprocessor.run_case(files, seg_prev_stage, None, self.plans_manager,
...
def predict_from_raw_data(list_of_lists_or_source_folder: Union[str, List[List[str]]],
                          ...
                          folder_with_segs_from_prev_stage2: str = None,
                          num_parts: int = 1,
                          part_id: int = 0,
    ...
    if configuration_manager.previous_stage_name2 is not None:
        if folder_with_segs_from_prev_stage2 is None:
            print(f'WARNING: The requested configuration is a cascaded model and requires predctions from the '
                  f'previous stage! folder_with_segs_from_prev_stage was not provided. Trying to run the '
                  f'inference of the previous stage...')
            folder_with_segs_from_prev_stage2 = join(output_folder,
                                                    f'prediction_{configuration_manager.previous_stage_name2}')
            predict_from_raw_data(list_of_lists_or_source_folder,
                                  folder_with_segs_from_prev_stage2,
                                  get_output_folder(plans_manager.dataset_name,
                                                    trainer_name,
                                                    plans_manager.plans_name,
                                                    configuration_manager.previous_stage_name2),
                                  use_folds, tile_step_size, use_gaussian, use_mirroring, perform_everything_on_gpu,
                                  verbose, False, overwrite, checkpoint_name,
                                  num_processes_preprocessing, num_processes_segmentation_export, None,
                                  num_parts=num_parts, part_id=part_id, device=device)
    ...
    seg_from_prev_stage_files2 = [join(folder_with_segs_from_prev_stage2, i + dataset_json['file_ending']) if
                                 folder_with_segs_from_prev_stage2 is not None else None for i in caseids]
    ...
    if not overwrite:
        ...
        seg_from_prev_stage_files2 = [seg_from_prev_stage_files2[i] for i in not_existing_indices]
    ...
    print(num_processes)
    ppa = PreprocessAdapter(list_of_lists_or_source_folder, seg_from_prev_stage_files, seg_from_prev_stage_files2, preprocessor,
    ...
    # mta = SingleThreadedAugmenter(ppa, NumpyToTensor(), pin_memory=device.type == 'cuda')
                    ...
                    try:
                        for params in parameters:
                            ...
                            if prediction is None:
                                print("Prediction is Nonen!!!! trying predict sliding window")
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('-prev_stage_predictions2', type=str, required=False, default=None,
                        help='Folder containing the predictions of the previous stage2. Required for cascaded models.')
    ...
    predict_from_raw_data(args.i,
                          ...
                          folder_with_segs_from_prev_stage2=args.prev_stage_predictions2,
                          device=device)
