...
def center_crop(data, crop_size, seg=None, focus=None):
    return crop(data, seg, focus, crop_size, 0, 'center')
...
def crop(data, seg=None, focus=None, crop_size=128, margins=(0, 0, 0), crop_type="center",
    ...
    if focus is not None:
        focus_shape = tuple([len(focus)] + list(focus[0].shape))
        focus_dtype = focus[0].dtype
    ...
    if focus is not None:
        focus_return = np.zeros([focus_shape[0], focus_shape[1]] + list(crop_size), dtype=focus_dtype)
    else:
        focus_return = None
    ...
    for b in range(data_shape[0]):
        ...
        if focus is not None:
            focus_shape_here = [focus_shape[0]] + list(focus[b].shape)
        ...
        if focus_return is not None:
            slicer_focus = [slice(0, focus_shape_here[1])] + [slice(lbs[d], ubs[d]) for d in range(dim)]
            focus_cropped = focus[b][tuple(slicer_focus)]
        ...
        if any([i > 0 for j in need_to_pad for i in j]):
            ...
            if focus_return is not None:
                focus_return[b] = np.pad(focus_cropped, need_to_pad, pad_mode_seg, **pad_kwargs_seg)
        else:
            data_return[b] = data_cropped
            if seg_return is not None:
                seg_return[b] = seg_cropped
            if focus_return is not None:
                focus_return[b] = focus_cropped

    return data_return, seg_return, focus_return


def random_crop(data, seg=None, focus= None, crop_size=128, margins=[0, 0, 0]):
    return crop(data, seg, focus, crop_size, margins, 'random')
