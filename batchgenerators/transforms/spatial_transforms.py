...
class SpatialTransform(AbstractTransform):
    ...
    def __call__(self, **data_dict):
        ...
        print("focus max and min:", np.max(focus), np.min(focus))
        ret_val = augment_spatial(data, seg, focus, patch_size=patch_size,
        ...
        print("focus after max and min:", np.max(ret_val[2]), np.min(ret_val[2]))
        data_dict[self.data_key] = ret_val[0]
        data_dict['focus'] = ret_val[2]
        if seg is not None:
