...
class nnUNetTrainer_20epochs_3Channel(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.is_cascaded2 = self.configuration_manager.previous_stage_name2 is not None
        self.is_cascaded3 = self.configuration_manager.previous_stage_name3 is not None
        ...
        self.folder_with_segs_from_previous_stage2 = \
            join(nnUNet_results, self.plans_manager.dataset_name,
                 self.__class__.__name__ + '__' + self.plans_manager.plans_name + "__" +
                 self.configuration_manager.previous_stage_name2, 'predicted_next_stage2', self.configuration_name) \
                 if self.is_cascaded2 else None

        self.folder_with_segs_from_previous_stage3 = \
            join(nnUNet_results, self.plans_manager.dataset_name,
                 self.__class__.__name__ + '__' + self.plans_manager.plans_name + "__" +
                 self.configuration_manager.previous_stage_name3, 'predicted_next_stage3', self.configuration_name) \
                if self.is_cascaded3 else None
        ...
        self.print_to_log_file('Cascade state:' + str(self.is_cascaded))
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            dataset = nnUNetDataset(self.preprocessed_dataset_folder, case_identifiers=None,
                                    ...
                                    folder_with_segs_from_previous_stage2=self.folder_with_segs_from_previous_stage2,
                                    folder_with_segs_from_previous_stage3=self.folder_with_segs_from_previous_stage3)
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for k in dataset_val.keys():
                ...
                if self.is_cascaded:
                    self.print_to_log_file('get cascaded!!!')
                    data = np.vstack((data, convert_labelmap_to_one_hot(seg[-2], self.label_manager.foreground_labels,
                                                                        output_dtype=data.dtype)))
                    self.print_to_log_file("Try to stack stage 2")
                    data = np.vstack((data, convert_labelmap_to_one_hot(seg[-1], self.label_manager.foreground_labels,
                                                                        output_dtype=data.dtype)))

                    ''' self.print_to_log_file("Try to stack stage 3")
                    data = np.vstack((data, convert_labelmap_to_one_hot(seg[-1], self.label_manager.foreground_labels,
                                                                        output_dtype=data.dtype))) '''
