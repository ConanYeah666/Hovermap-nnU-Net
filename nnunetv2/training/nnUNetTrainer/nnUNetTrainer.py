...
from nnunetv2.training.loss.hovloss import HovLoss
...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.loss = self._build_loss(hov=True)
            self.was_initialized = True
            print("Done with initialzied")
        else:
    ...
    def _build_loss(self, hov:bool = False):
        print("If we need to build loss?")
        if hov:
            loss = HovLoss({'batch_dice': self.configuration_manager.batch_dice,
    ...
    def get_dataloaders(self):
        ...
        patch_size = self.configuration_manager.patch_size
        dim = len(patch_size)
        print("Get patch size:",patch_size)
        ...
        tr_transforms = self.get_training_transforms(
            ...
            ignore_label=self.label_manager.ignore_label)
        print("After getting tr transform")
        ...
        val_transforms = self.get_validation_transforms(deep_supervision_scales,
                                                        ...
                                                        ignore_label=self.label_manager.ignore_label)
        print("After getting val transform")
        dl_tr, dl_val = self.get_plain_dataloaders(initial_patch_size, dim)
        print("After get plain dataloaders")

        allowed_num_processes = 0
        print(allowed_num_processes)
        if allowed_num_processes == 0:
            print("did mt gen train")
            mt_gen_train = SingleThreadedAugmenter(dl_tr, tr_transforms)
            print("did mt gen val")
    ...
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        ...
        if dim == 2:
            ...
            dl_val = nnUNetDataLoader3D(dataset_val, self.batch_size,
                                        ...
                                        oversample_foreground_percent=self.oversample_foreground_percent,
                                        sampling_probabilities=None, pad_sides=None)
        print("DONE GETTTING dataloader")
        return dl_tr, dl_val
    ...
    def get_training_transforms(patch_size: Union[np.ndarray, Tuple[int]],
        ...
        if do_dummy_2d_data_aug:
            ...
            ignore_axes = None
        print("After doing dummy_2d_data aug.")
        ...
        print("Step in tr transform Appending")
        ...
        if deep_supervision_scales is not None:
            tr_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                              output_key='target'))
            tr_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='focus',
                                                              output_key='focus'))

        tr_transforms.append(NumpyToTensor(['data', 'target', 'focus'], 'float'))
    ...
    def get_validation_transforms(deep_supervision_scales: Union[List, Tuple],
        ...
        if deep_supervision_scales is not None:
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                               output_key='target'))
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='focus',
                                                               output_key='focus'))

        val_transforms.append(NumpyToTensor(['data', 'target', 'focus'], 'float'))
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        target = batch['target']
        focus = batch['focus']
        # print("focus: ", torch.max(focus[0]), torch.min(focus[0]))
        #print("target: ", torch.max(target[0]), torch.min(target[0]))
        #print("data: ", torch.max(data[0]), torch.min(data[0]))
        ...
        if isinstance(focus, list):
            focus = [i.to(self.device, non_blocking=True) for i in focus]
        else:
            focus = focus.to(self.device, non_blocking=True)
        ...
        with torch.cuda.amp.autocast() if self.device.type == 'cuda' else dummy_context():
            #print(data.shape)
            #print("Staring network")
            output = self.network(data)
            # del data
            #print("Staring bcompute loss")
            l = self.loss(output, target, focus)

        if self.grad_scaler is not None:
            print("Staring backward grad")
            ...
            print("Staring backward")
            l.backward()
            torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
            self.optimizer.step()
        print("I've already sent back")
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if self.label_manager.has_regions:
            ...
            predicted_segmentation_onehot= output_seg
            del output_seg
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for k in dataset_val.keys():
                ...
                data, seg, focus, properties = dataset_val.load_case(k)
                ...
                del data
                gc.collect()
                self.print_to_log_file(prediction)
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                ...
                t1 = t.time()
                current = next(self.dataloader_train)
                t2 = t.time()
                print(current.keys())
                # self.print_to_log_file("Time is:", t2-t1)
                train_outputs.append(self.train_step(current))
                print("train output:", train_outputs)
                t3 = t.time()
                # self.print_to_log_file("Time2: ", t3-t2)
                del current
                gc.collect()
                print("batchid:", batch_id)
            print("Starting next step")
            self.print_to_log_file(train_outputs)
            ...
            with torch.no_grad():
                ...
                for batch_id in range(self.num_val_iterations_per_epoch):
                    ...
                    t2 = t.time()
                    print("batchid:", batch_id, "Time: ", t2 - t1)
