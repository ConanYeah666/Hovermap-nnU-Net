...
def maybe_mirror_and_predict(network: nn.Module, x: torch.Tensor, focus: torch.Tensor = None, mirror_axes: Tuple[int, ...] = None) \
        -> torch.Tensor:
    prediction = network(x)
    print("x shape:", x.shape)
    print("prediction shape:", prediction.shape)
    if focus is not None:
        prediction = prediction * focus

    print("Torch min and max:", torch.max(prediction), torch.min(prediction))
    print("Maybe mirror _predict axes", mirror_axes)
    mirror_axes = None
...
def predict_sliding_window_return_logits(network: nn.Module,
                                         ...
                                         tile_size: Tuple[int, ...],
                                         focus: Union[np.ndarray, torch.Tensor] = None,
    ...
    with torch.no_grad():
        ...
        with torch.autocast(device.type, enabled=True) if device.type == 'cuda' else dummy_context():
            ...
            if focus is not None:
                focus, slicer_revert_padding2 = pad_nd_image(focus, tile_size, 'constant', {'value': 0}, True, None)
                focus = focus.reshape((1,) + focus.shape)
                focus = torch.cat([focus, focus, focus], axis=0)
                print(focus.shape)
            print(data.shape)
            print("Data max min:", torch.max(data), torch.min(data))
            if use_gaussian:
            ...
            try:
                ...
                print(predicted_logits.shape, n_predictions.shape, gaussian.shape)
            except RuntimeError:
                ...
                n_predictions = torch.zeros(data.shape[1:], dtype=torch.half,
                                            device=results_device)
                gaussian = gaussian.to(results_device)
                print('fall back:', predicted_logits.shape, n_predictions.shape, gaussian.shape)
            finally:
                empty_cache(device)
            ...
            for sl in slicers:
                ...
                if focus is not None:
                    empty_cache(device)
                    focus2 = focus[sl][None]
                    focus2 = focus2.to(device, non_blocking=False)
                else:
                    focus2 = None
                prediction = maybe_mirror_and_predict(network, workon, focus2, mirror_axes)[0].to(results_device)
                print('prediction:', prediction.shape)
                print("prediction max min:", torch.max(prediction), torch.min(prediction))
