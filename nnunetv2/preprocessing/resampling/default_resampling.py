...
import numpy as np
import pandas as pd
from batchgenerators.augmentations.utils import resize_segmentation,resize_segmentation_hov
...
def resample_data_or_seg(data: np.ndarray, new_shape: Union[Tuple[float, ...], List[float], np.ndarray],
    ...
    if is_seg:
        resize_fn = resize_segmentation_hov
        kwargs = OrderedDict()
    else:
        resize_fn = resize
        kwargs = {'mode': 'edge', 'anti_aliasing': False}
    dtype_data = np.float16 #data.dtype
    shape = np.array(data[0].shape)
    new_shape = np.array(new_shape)
    print("Current new shape is:", new_shape)
    if np.any(shape != new_shape):
        ...
        if do_separate_z:
            ...
            for c in range(data.shape[0]):
                ...
                # print(data.dtype)
                reshaped_data1 = []
                print(shape[axis], new_shape[axis])
                print(axis)
                for slice_id in range(shape[axis]):
                    if axis == 0:
                        reshaped_data1.append(resize_fn(data[c, slice_id], new_shape_2d, order, **kwargs))
                    elif axis == 1:
                        reshaped_data1.append(resize_fn(data[c, :, slice_id], new_shape_2d, order, **kwargs))
                    else:
                        reshaped_data1.append(resize_fn(data[c, :, :, slice_id], new_shape_2d, order, **kwargs))
                # print('t1:', time.time()-t1)
                print("Done slice and reshaped append")
                gc.collect()
                reshaped_data = np.stack(reshaped_data1, axis)
                del reshaped_data1
                gc.collect()
                ...
                if shape[axis] != new_shape[axis]:
                    ...
                    if c == 0:
                        ...
                        # print(orig_rows, orig_cols, orig_dim)
                        # print(rows, cols, dim)
                        ...
                        print("rows, cols, dims:" , map_rows, map_cols, map_dims)
                        # print(map_rows.shape, map_cols.shape, map_dims.shape)
                        coord_map = np.array([map_rows, map_cols, map_dims], dtype=float)
                    ...
                    if not is_seg or order_z == 0:
                        print(coord_map.shape)
                        print(type(coord_map))
                        print(reshaped_data.shape)
                        print(type(reshaped_data))
                        reshaped_final_data.append(map_coordinates(reshaped_data.astype(float), coord_map, order=order_z,
                                                                   mode='nearest')[None])
                    else:
                        unique_labels = np.sort(pd.unique(reshaped_data.ravel()))  # np.unique(reshaped_data)
                        print("curr label:", unique_labels.shape)
                        print(unique_labels)
                        reshaped = np.zeros(new_shape, dtype=dtype_data)
                        reshaped = map_coordinates(reshaped_data.astype(float), coord_map, order=order_z, mode='nearest')
                        reshaped_final_data.append(reshaped[None])
                    print('t_final:', time.time()-t1)
                else:
                    reshaped_final_data.append(reshaped_data[None])
                gc.collect()
            reshaped_final_data_1 = np.vstack(reshaped_final_data)
            del reshaped_final_data
            gc.collect()
            reshaped_final_data = reshaped_final_data_1
