...
def convert_seg_image_to_one_hot_encoding(image, classes=None):
    ...
    for i, c in enumerate(classes):
        print("Convert seg image to one hot encoding")
...
def interpolate_img(img, coords, order=3, mode='nearest', cval=0.0, is_seg=False):
    if is_seg and order != 0:
        ...
        for i, c in enumerate(unique_labels):
            #print(c)
            print("image unique:", np.unique(img), np.count_nonzero(img[img == 1]), np.count_nonzero(img[img == 1])/np.prod(img.shape))

            res_new = map_coordinates(img.astype(float), coords, order=order, mode=mode, cval=cval).astype(img.dtype)
            print("res unique:",np.unique(img), np.count_nonzero(img[img != 1]) - np.count_nonzero(img[img<0]))
            result[res_new > 0] = c
        print("final unique:", np.unique(result), np.count_nonzero(result[result == 1]))
        return result
    else:
        #print("first unique:", np.unique(img), np.count_nonzero(img))
        newim = map_coordinates(img.astype(float), coords, order=order, mode=mode, cval=cval).astype(img.dtype)

        print("after unique:", np.unique(newim), np.count_nonzero(newim))
        return newim
...
def resize_segmentation_hov(segmentation, new_shape, order=3):
    '''
    Resizes a segmentation map. Supports all orders (see skimage documentation). Will transform segmentation map to one
    hot encoding which is resized and transformed back to a segmentation map.
    This prevents interpolation artifacts ([0, 0, 2] -> [0, 1, 2])
    ...
    if order == 0:
        ...
        reshaped =  resize(segmentation.astype(float), new_shape, order, mode="edge", clip=True, anti_aliasing=False)
        return reshaped
